 ...
 */
export type RasterMode = AnyCase<
  "normal" | "dw" | "dh" | "dwdh" | "dhdw" | "dwh" | "dhw"
>;
...
export type StyleString = AnyCase<
  "normal" | `${"b" | ""}${"i" | ""}${"u" | "u2" | ""}`
>;

export type FeedControlSequence = AnyCase<
  "lf" | "glf" | "ff" | "cr" | "ht" | "vt"
>;
...
export class Printer<AdapterCloseArgs extends []> extends EventEmitter {
  ...
  tableCustom(
    ...
    options: CustomTableOptions = { size: [1, 1], encoding: this.encoding }
  ): this {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if (align === "CENTER") {
        ...
        if (obj.text !== "") {
          if (obj.style)
            lineStr += `${this._getStyle(obj.style)}${obj.text}${this._getStyle(
              "NORMAL"
            )}`;
  ...
  control(ctrl: FeedControlSequence) {
    this.buffer.write(
      _.FEED_CONTROL_SEQUENCES[`CTL_${utils.upperCase(ctrl)}` as const]
    );
    return this;
  }
  /**
  ...
  align(align: Alignment) {
    this.buffer.write(
      _.TEXT_FORMAT[`TXT_ALIGN_${utils.upperCase(align)}` as const]
    );
    return this;
  }
  /**
  ...
  font(family: FontFamily) {
    this.buffer.write(
      _.TEXT_FORMAT[`TXT_FONT_${utils.upperCase(family)}` as const]
    );
  ...
  _getStyle(
    boldOrString: boolean | StyleString,
    italic?: boolean,
    underline?: boolean | 0 | 1 | 2
  ) {
  ...
  barcode(
    ...
    this.buffer.write(
      codeLength +
        convertCode +
        (options.includeParity ? parityBit : "") +
        "\x00"
  ...
  qrcode(
    ...
    if (this._model !== "qsprinter") {
      ...
      this.buffer.write(
        _.CODE2D_FORMAT[`QR_LEVEL_${utils.upperCase(level ?? "L")}` as const]
      );
      ...
      this.buffer.write(
        _.MODEL.QSPRINTER.CODE2D_FORMAT.LEVEL.OPTIONS[
  ...
  qrimage(
    text: string,
    options: QrImageOptions = { type: "png", mode: "dhdw" }
  ): Promise<this> {
  ...
  async image(image: Image, density: BitmapDensity = "d24") {
    ...
    const header =
      _.BITMAP_FORMAT[`BITMAP_${utils.upperCase(density)}` as const];
   ...
   */
  getStatus<T extends DeviceStatus>(
    statusClass: StatusClassConstructor<T>
  ): Promise<T> {
  ...
  /**
   * Set mode to support kanji (Japanese & Chinese) characters
   */
  setKanjiMode = function () {
    this.buffer.write(_.CHARACTER_SET.KANJI);
    return this;
  };
   ...
   * Cancel kanji mode
   */
  cancelKanjiMode = function () {
    this.buffer.NORMAL(_.CHARACTER_SET.KANJI);
    return this;
  };
}
